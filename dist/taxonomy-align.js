...
function isParkingCategory(cat, domainNames) {
    return cat.name === 'Misc' && (domainNames.get(cat.domain_id) ?? '') === 'General';
}
export function findMergeCandidates(db, opts = {}) {
    ...
    const countRows = db
        .prepare(`SELECT ontology_category_id AS id, COUNT(*) AS n
    ...
    try {
        ...
        knnStmt = db.prepare(`SELECT id, distance FROM vec_categories WHERE embedding MATCH ${vecParamSql(dt)} ORDER BY distance LIMIT ?`);
    }
    catch {
    ...
    for (const cat of categories) {
        ...
        let hits;
        try {
            hits = knnStmt.all(embRow.embedding, NEIGHBORS_PER_CATEGORY + 1);
        }
        catch {
            continue;
        }
        for (const hit of hits) {
            ...
            if (!other)
                continue; // stale vec row (category already deleted)
            const pairKey = [cat.id, other.id].sort().join('|');
            if (seenPairs.has(pairKey))
                continue;
            seenPairs.add(pairKey);
            if (isParkingCategory(cat, domainNames) || isParkingCategory(other, domainNames))
                continue;
            ...
            const keepFirst = catCount > otherCount || (catCount === otherCount && cat.created_at <= other.created_at);
 ...
 * scoring.
 */
export function applyMerges(db, candidates) {
    const remap = new Map();
    const resolve = (id) => {
        let cur = id;
        const seen = new Set();
    ...
    const countStmt = db.prepare('SELECT COUNT(*) AS n FROM facts WHERE is_active = 1 AND ontology_category_id = ?');
    const mergeTx = db.transaction((keepId, dropId) => {
        const res = db
    ...
    const result = { merged: 0, factsRemapped: 0, skippedCrossDomain: 0, skippedStale: 0 };
    for (const cand of candidates) {
        ...
        const aRow = catStmt.get(a);
        const bRow = catStmt.get(b);
        if (!aRow || !bRow) {
            result.skippedStale++;
            continue;
        }
        ...
        const aCount = countStmt.get(a).n;
        const bCount = countStmt.get(b).n;
...
export function formatAlignmentReport(find, opts, applied) {
    ...
    const line = (c) => `- ${c.similarity.toFixed(3)} [${c.keepDomain}] "${c.dropName}" (${c.dropFactCount} facts) → merge into "${c.keepName}" (${c.keepFactCount} facts)${c.sameDomain ? '' : ` — cross-domain: [${c.dropDomain}] vs [${c.keepDomain}]`}\n`;
    if (sameDomain.length > 0) {
        out += `## Same-domain merge candidates\n\n`;
        for (const c of sameDomain.slice(0, opts.show))
            out += line(c);
    ...
    if (crossDomain.length > 0) {
        ...
        for (const c of crossDomain.slice(0, Math.min(10, opts.show)))
            out += line(c);
