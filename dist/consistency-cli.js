...
import { formatConsistencyReport, getConsistencyCounts, hasActiveConflicts, listActiveConflicts, } from './consistency.js';
import { countActivePrincipleConflicts, formatPrincipleConflictSection, listActivePrincipleConflicts, } from './principles.js';
...
function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (arg === '--json')
            ...
            opts.gate = true;
        else if (arg === '--gate-principles')
            opts.gatePrinciples = true;
        else if (arg === '--limit') {
...
function main() {
    ...
    try {
        ...
        if (opts.json) {
            ...
            const principleSection = formatPrincipleConflictSection(principleConflictCount, principleConflicts, principleCoverage);
            if (principleSection)
                report += `\n${principleSection}`;
            console.log(report);
        }
        if (opts.gate && hasActiveConflicts(counts)) {
            console.error(`consistency gate: ${counts.activeContradictsPairs} CONTRADICTS + ${counts.activeSupersedesPairs} SUPERSEDES active-active pairs`);
            process.exitCode = 2;
        }
