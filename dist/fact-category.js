...
const CATEGORY_SET = new Set(FACT_CATEGORIES);
export function isFactCategory(value) {
 ...
 */
export function normalizeFactCategory(raw) {
    if (typeof raw === 'string') {
        const v = raw.trim().toLowerCase();
        if (CATEGORY_SET.has(v))
            return v;
        if (v === 'requirement' || v === 'requirements')
            return 'constraint';
        if (v.includes('|')) {
