...
export function classifyLlmError(err) {
    ...
    if (/unauthor|forbidden|invalid.*(api.?key|access.?token|credential)|timeout|etimedout|econnreset|econnrefused|enotfound|socket hang up|network|overloaded|temporarily|rate.?limit|too many requests|service unavailable|internal server error|bad gateway|gateway timeout/.test(m)) {
        return 'transient';
    }
