...
function main() {
    ...
    try {
        ...
        console.log(`remaining: ${result.remaining} (embedding 보유) / unmeasurable: ${result.unmeasurable} (embedding 없음 — NULL 유지)`);
        if (result.distribution) {
            const d = result.distribution;
            console.log(`distribution (n=${d.count}): min ${d.min.toFixed(3)} | p25 ${d.p25.toFixed(3)} | median ${d.median.toFixed(3)} | p75 ${d.p75.toFixed(3)} | max ${d.max.toFixed(3)}`);
        }
