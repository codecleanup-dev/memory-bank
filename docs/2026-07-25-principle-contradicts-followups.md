...
### 실측 결과 (2026-07-25 — **양성, 셔플 구현 확정**)

동일 첫 200 facts, 단일표 4런 (S=canonical ×2, O=셔플 seed 41/97), 판정 셋 크기 11/10/11/9:

| 비교 | Jaccard |
|---|---|
| 동일 순서 J(S1,S2) | **0.400** |
| 교차 순서 J(S1,O1)/J(S1,O2)/J(S2,O1)/J(S2,O2) | 0.222 / 0.250 / 0.167 / 0.118 |
| 셔플 간 J(O1,O2) | 0.333 |
| 교차 평균 | **0.218** |
| **orderEffectDelta** | **0.182 > 임계 0.10 → 양성** |

- 해석: 순서를 바꾸면 판정 겹침이 절반으로 떨어진다 — 단일표 churn의 상당분이 확률 노이즈가
  아니라 **계통적 순서 효과**. 동일 순서 위원회 3표는 이 편향을 공유하므로 다수결이 못 거른다.
- 조치: `committeeJudge`에 **표별 배치 셔플** 도입 — 1표는 호출자 순서 유지, 2표부터 독립 순열,
  finding의 `fact_index`는 호출자 순서로 역매핑(하류 검증·tally 의미 불변). 순서-계통 편향이
  표간 분산으로 바뀌어 다수결이 누른다.
- 부수 관찰: 동일 순서 재현율 0.400 자체도 낮음(단일표의 본질적 불안정) — 위원회 다수결의
  필요성을 재확인. 셔플+다수결 결합 후의 재현율 개선은 다음 200-facts 재판정에서 추적.
