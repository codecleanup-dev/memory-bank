...
> 작성: 2026-07-06 · 상위: 내부 마스터 플랜 문서(비공개)
> 성격: 마스터 플랜 방안 C(AI 오피스) C-1 인벤토리 감사의 프로젝트 실행분. **실행은 항목별 사람 승인.**

## 오피스 좌표
- 부서: **지식실(정본)** — 전 세션 대화·판단의 자동 색인·검색. Claude/Codex 양쪽의 메모리 정본 (Codex 메모리 정본 = memory-bank — 내부 운영 판정)
...
- 최근 실전 교훈: 플러그인 native 모듈(better-sqlite3) ABI ↔ 런처 node 불일치가 MCP 읽기 경로만 11일 침묵 사망 — toolchain-smoke에 probe 추가됨 (내부 하네스에 반영 완료)
...
| 오피스(방안 C) | 지식실 정본 — 내부 지식 도구 레인이 전부 이 DB를 소비 |
