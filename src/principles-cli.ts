...
const USAGE = `Usage: memory-bank principles <list|add|import|activate|deactivate|conflicts|resolve|check> [options]
  ...
  check       Scan active facts with the LLM judge [--dry-run] [--max-facts N] [--batch-size N] [--threshold T] [--recheck] [--json]`;
...
function intOpt(opts: Opts, name: string, fallback: number): number {
...
function requireValue(opts: Opts, name: string): string {
    ...
    switch (sub) {
      case 'list': {
        ...
        if (json) {
          ...
          for (const p of principles) {
            ...
            if (p.source_path) console.log(`  source: ${p.source_path}`);
          }
        }
        break;
      }
      ...
      case 'add': {
        const principle = addPrinciple(db, {
          ...
          layer: (opts.values.get('--layer') as PrincipleLayer | undefined) ?? 'principle',
        });
      ...
      case 'import': {
        ...
        let entries: Array<{ slug: string; statement: string; source?: string; layer?: PrincipleLayer }>;
        try {
      ...
      case 'resolve': {
        ...
        const resolution = requireValue(opts, '--resolution') as ConflictResolution;
      ...
      case 'check': {
        ...
        if (json) {
          ...
          console.log(
            `checked ${result.factsChecked} facts in ${result.batches} batches against ${result.activePrinciples} principles`,
          );
          ...
          if (result.error) console.error(`judge error (run stopped, batch will retry next run): ${result.error}`);
        }
