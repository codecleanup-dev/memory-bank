...
export function exportForSync(): { facts: number; domains: number; categories: number; relations: number } {
  ...
  try {
    ...
    // instead of silent.
    const relationsTotal = (db.prepare('SELECT COUNT(*) AS n FROM ontology_relations').get() as { n: number }).n;
    ...
    if (relationsTotal > relations.length) {
      console.error(
        `sync-export: ${relationsTotal - relations.length} relations excluded (endpoint inactive or missing — local history only).`,
      );
    }
