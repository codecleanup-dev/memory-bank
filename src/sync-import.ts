...
interface SyncFact {
  ...
  confidence?: number | null; // absent in files from pre-confidence exporters
}
...
export async function importFromSync(): Promise<{ newFacts: number; newDomains: number; newCategories: number; newRelations: number }> {
  ...
  try {
    ...
    const seenInBatch = new Map<string, string>(); // contentKey → surviving remote id
    ...
    for (const line of factsLines) {
      try {
        ...
        `).get(f.fact, f.scope_type, f.scope_project ?? '') as { id: string } | undefined;
        ...
        try {
          ...
          const insertTx = db.transaction(() => {
            ...
            `).run(
              ...
              f.scope_type, f.scope_project, f.source_exchange_ids,
    ...
    if (fs.existsSync(relationsPath)) {
      ...
      for (const line of lines) {
        try {
          ...
          if (relationExistsBetween(db, source, target, r.relation_type as RelationType)) continue;
